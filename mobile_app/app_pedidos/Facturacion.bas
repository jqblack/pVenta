...
Sub cargarProductosTemporales
	...
	If Main.logged And 1==2 Then
		
		Dim json As String  = "'f_idempresa':"&Principal.idEmpresa& _
	" ,'f_idusuario':"&Main.usersData.idUsuario
	
		Dim JobBuscar As HttpJob
		JobBuscar.Initialize("JobBuscar",Me)
		JobBuscar.PostString(Main.url&"puntoventa/get_productos_temporalesLogged","json="&funcion.crearJson(Main.token,json))
	...
	" ,'f_imei':"&Main.imei
		...
		Dim JobBuscar As HttpJob
		JobBuscar.Initialize("JobBuscar",Me)
		JobBuscar.PostString(Main.url&"puntoventa/get_productos_temporales_by_key2","json="&funcion.crearJson(Main.tokenPublico,json))
	End If
	...
	Select Job.JobName
		...
		Case "JobBuscar"
			...
			If Main.logged Then
				...
				lblOrdenTotalPagar.Text = "$"& NumberFormat(funcion.getDecimal(total+ Principal.mapEmpresaInfo.Get("f_precio_envio")),2,2)
					
					Else
				'lblOrdenTotalPagar.Text = "$"& NumberFormat(funcion.getDecimal(total),2,2)
				lblOrdenTotalPagar.Text = "$"& NumberFormat2(total,0,2,2,False)
				ToastMessageShow("Dicho precio esta calculado SIN el precio de envio de la empresa correspondiente",False)
			End If
		...
		Case "JobDirecciones"
			...
			For i=0 To list.Size -1
				...
				ListViewDirecciones.AddTwoLines2(dr,DateTime.Date(map.Get("f_fecha")) & default ,map)
			Next
...
Sub btnOrdenEnviar_Click
	...
	If Main.logged Then
		...
		Dim res As Int = Msgbox2("Seguro que desea realizar este pedido?","Confirmación","Si","","No",Null)
		
		If res = DialogResponse.POSITIVE Then
			
			Dim json As String
			json = "'f_comprobante_fiscal':false,'f_idempresa':"&Principal.idEmpresa&",'f_itbis':"&tax&",'f_subtotal':"&subtotal&",'f_total':"&total&",'f_nota':'','f_imei':"&Main.imei&",'f_direccion':'"&funcion.base64_Encode(lblDireccion.Text)&"'"
	
			Dim JobSalvar As HttpJob
			JobSalvar.Initialize("JobSalvar",Me)
			JobSalvar.PostString(Main.url&"puntoventa/set_salvar_orden_delivery2/","json="&funcion.crearJson(Main.token,json))
	...
	Else
		...
		res = Msgbox2("Para hacer un pedido primero debe iniciar Seción","Desea iniciar seción","Si","","Mas tarde",Null)
...
Sub ListViewOrdenProducto_ItemClick (Position As Int, Value As Object)
...
Sub PanelDireccion_Click
	...
	Else
		...
		res = Msgbox2("Para agregar o ver tus direcciones primero debe iniciar Seción","Desea iniciar seción","Si","","Mas tarde",Null)
...
Sub btnOrdenEliminarTodos_Click
	...
	JobEliminarTodo.PostString(Main.url&"puntoventa/get_eliminar_detalle_usuarios2","json="&funcion.crearJson(Main.tokenPublico,json))
End Sub
