...
class ConfiguracionServices {
                ...
                tdetallesTemporal.each { e ->
                    ...
                    if (!tproducto.isEmpty()) {
                        ...
                        parametros_detalles = [
                                ...
                                false,
                                idcomanda,
                                new String(e.f_comentario.toString().decodeBase64())
    ...
    @Transactional
    def InsertarDireccion(String id, String direccion, Long idusuario,Boolean principal) {
        String sql = "";
        Object [] parametros = [] as Object;
        if (!id.equals("0"))
        {
            sql = "UPDATE  movil.t_direcciones_usuarios set f_direccion = ?, f_principal = ?  WHERE f_idusuario = ?";
            parametros = [direccion,principal,idusuario] as Object;
        }else{

            sql = "insert  into movil.t_direcciones_usuarios(f_idusuario,f_direccion,f_principal) values(?,?,?) ";
            parametros = [idusuario,direccion,principal] as Object;
        }
    ...
    @Transactional
    def crearContactosClientes(Long idusuario,String lista_contactos){
        JsonSlurper json = new JsonSlurper();

        List<Map<String,Object>> list = json.parseText(lista_contactos);
        list.each {e->
            Object[] parametros =[e.f_id.toString().toLong(),idusuario] as Object;
            String sql = "select * from movil.t_persona_contacto where f_id = ? and f_idusuariomovil = ? ";
            Map tverificar = sqlService.GetQueryMap(sql,parametros);
            if (tverificar.isEmpty())
            {
                parametros =[idusuario,new String(e.f_nombre.toString().decodeBase64()),e.f_id.toString().toLong()] as Object;
                sql = "insert into movil.t_persona_contacto (f_idusuariomovil, f_nombre,f_id) VALUES (?,?,?) returning *";
                Map tpersona = sqlService.GetQueryMap(sql,parametros);
                List<Map<String,Object>> telefonos =(List) e.f_phone_list;
                telefonos.each {t ->
                    parametros = [
                            tpersona.f_idrecord.toString().toLong(),
                            t.f_phone_number.toString(),
                            t.f_type.toString().toLong()
                    ] as Object
                    sql = "INSERT into movil.t_personas_telefonos (f_idpersona, f_telefono, f_tipo) values (?,?,?)";
                    sqlService.EjecutarQuery(sql,parametros);
                }
            }
        }
