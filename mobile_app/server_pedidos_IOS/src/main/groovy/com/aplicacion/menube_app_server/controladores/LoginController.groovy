...
////        list.add(map); list.add(map);list.add(map);list.add(map);
////        String[] campos = ["f_idrecord", "f_nombre",];
////        String[] valore = [funciones.ListToJson(list), funciones.ListToJson(list)]
    ...
    def login(@RequestParam(name = "json") String dato) {
        ...
        Map tupla = JSON.parse(map.get("f_data").toString());
            ...
            if (list.size() > 0) {
                ...
                mapa.put("f_idrecord",map_usuario.get("f_idrecord").toString());
                mapa.put("f_nombre", map_usuario.get("f_nombre").toString());
                mapa.put("f_latitud",map_usuario.get("f_latitud").toString());
                mapa.put("f_longitud", map_usuario.get("f_longitud").toString());
                mapa.put("f_lugares_favoritos",funciones.ListToJson(lista_lugares_favoritos));

                //return funciones.convertir(token, funciones.crearJSON(campos, valores)).replace("'", '"').replace('"[', '[').replace(']"', ']');
                return funciones.convertir(token, mapa);
            } else {
                /*******Usuario Desactivado*/
